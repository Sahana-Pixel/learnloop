...
import { Menu, X, Home, Users, Code, GraduationCap, Map, Calendar, Bookmark, LogIn, Rocket } from "lucide-react";
...
const Navbar: React.FC = () => {
  ...
  return (
   <nav className={`w-full bg-gradient-to-b from-gray-900 to-gray-900/80 text-gray-100 fixed top-0 z-50 backdrop-blur-lg border-b border-gray-800/50 transition-all duration-300 ${scrolled ? "pt-6 pb-2 shadow-xl" : "pt-8 pb-3 shadow-md"}`}>
...
>
  {/* Logo Mark with Shading */}
  <div className="relative">
    {/* Base shape with shading */}
  ...
  <span className="text-2xl font-bold text-white">
    <span className="bg-clip-text text-transparent bg-gradient-to-r from-gray-300 to-gray-300">
      LearnLoop
        ...
        {/* Desktop Menu - Centered with Animated Underline */}
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <a
                  href={item.href}
                  className={`flex flex-col items-center px-4 py-2 text-sm transition ${activeLink === item.href ? "text-blue-400" : "text-gray-300 hover:text-white"}`}
        ...
        {/* Right Side Buttons with Modern Style */}
              ...
              {navItems.map((item) => {
                ...
                return (
                  <motion.a
                    key={item.label}
                    href={item.href}
                    className={`flex items-center px-4 py-3 rounded-xl text-sm ${activeLink === item.href ? "bg-blue-900/30 text-blue-400" : "text-gray-300 hover:bg-gray-800 hover:text-white"}`}
                    onClick={() => {
